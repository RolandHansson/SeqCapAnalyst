...
while true
do
  if [ $first_time -eq 1 ] ; then #DO THIS ONLY THE FIRST TIME
    first_time=0; 
    [[ $verbose -eq 1 ]] && >&2 echo "FINDING POSITIONS!" 
    #Find first line with values in probe file
    while true
    do
	read -r scaffold name seq rep str pos trash <&3 || break
	[[ $verbose -eq 1 ]] && >&2 echo "pos: '$pos'"
	[[ $pos =~ [1-9] ]] && break
    done
    [[ $verbose -eq 1 ]] && >&2 echo "'$scaffold' '$name' '$seq' '$rep' '$str' '$pos' '$scaffold_nr' '$start'"

    #Find first line with values in exon file
    while true
    do
	read -r exon_pos gene_name gene_length <&4 || break 
	[[ $verbose -eq 1 ]] && >&2 echo "gene_length: '$gene_length'"
	[[ $gene_length =~ [0-9] ]] && break
    done
    read -r exon_scaffold exon_start exon_end trash <<< `echo "$exon_pos" | cut -d '(' -f1 | tr ":-" "  "`
    exon_scaffold_nr=`echo "$exon_scaffold" | sed "s/.*fold//"`
    [[ $verbose -eq 1 ]] && >&2 echo "'$exon_pos' '$gene_name' '$gene_length' '$exon_scaffold' '$exon_start' '$exon_end' '$exon_scaffold_nr'"
  else #DO THIS EVERYTIME EXCEPT FIRST TIME
      read -r scaffold name seq rep str pos trash <&3 || break
  fi
  ...
  #then #load actual first probe line
    ...
    #read -r exon_scaffold exon_start exon_end trash <<< `echo "$exon_pos" | cut -d '(' -f1 | tr ":-" "  "`
    #exon_scaffold_nr=`echo "$exon_scaffold" | sed "s/.*fold//"` 
    #[[ $verbose -eq 1 ]] && >&2 echo "'$exon_pos' '$gene_name' '$gene_length' '$exon_scaffold' '$exon_start' '$exon_end'"
    #[[ $verbose -eq 1 ]] && >&2 echo "'$scaffold' '$name' '$seq' '$rep' '$str' '$pos' '$scaffold_nr' '$start'"
  #fi
  pos=`echo $pos | sed "s/chrHt/Ht/"` 
  >&2 echo -e "\e[1A" "processing probe $name ($pos)                  "
  read scaffold_nr start end <<< `echo "$pos" | sed "s/.*fold//" | tr ":-" "  "` 
  [[ $verbose -eq 1 ]] && >&2 echo -en "pos=$pos\n start=$start\n end=$end, nr=$scaffold_nr\n"
  #Go to correct scaffold
  [[ $verbose -eq 1 ]] && >&2 echo "comparing probe scaffold '$scaffold_nr' vs exon scaffold '$exon_scaffold_nr'"
...
done 3< <( cat $probes_file | grep -v "^#" | grep . | sort -V -k6 ) 4< <( cat $exons_file | sort -k1 -V )
